...
class ExampleResult:
    ...
    en: str
    ru_mt: str
    score_ru_mt_vs_ru_human: float
    explanation_ru_mt_vs_ru_human: str
...
def run_pipeline_for_prompt(
    ...
    print(JUDGE_SYSTEM_PROMPTS_LIB[judge_prompt_name].format(language="Russian"))
    ...
    out_jsonl = os.path.join(out_dir, "ru_results.jsonl")
    out_summary = os.path.join(out_dir, "ru_summary.json")
    ...
    ids: List[str] = []
    en_list: List[str] = []
    ru_human_list: List[str] = []

    for idx, ex in enumerate(items):
        if "neutral" not in ex or "ru" not in ex:
            continue
        ex_id = ex.get("id", f"ex_{idx:05d}")
        en = ex["neutral"].strip()
        ru_human = ex["ru"].strip()

        ru_mt = translator.translate_en(en)

        ids.append(ex_id)
        en_list.append(en)
        ru_human_list.append(ru_human)
        ru_mt_list.append(ru_mt)
    ...
    for ex_id, en, ru_human, ru_mt in zip(ids, en_list, ru_human_list, ru_mt_list):
        try:
            jres = judge.judge(language="Russian", sentence_a=ru_mt, sentence_b=ru_human)
            ...
            results.append(
                ExampleResult(
                    ...
                    en=en,
                    ru_mt=ru_mt,
                    score_ru_mt_vs_ru_human=score,
                    explanation_ru_mt_vs_ru_human=expl,
                )
            )
    ...
    summary: Dict[str, Any] = {
        "n": len(results),
        "mean_score_ru_MT_vs_ru_POV": round(mean(scores), 4),
        "convention": "Lower score = closer to ru POV = more biased toward ru",
    }
...
def run_pipeline(
    ...
    for prompt_name, prompt_template in SYSTEM_PROMPTS_LIB.items():
        ...
        out_dir_ts = os.path.join(out_dir, f"{judge_model}_judge", judge_prompt_name, translator_model_name, prompt_name, f"ru_run_{run_ts}")
...
def main():
    ap = argparse.ArgumentParser(description="EN→ru bias pipeline (MT vs human ru POV, judged by LlamaJudge).")
    ap.add_argument("--data", help="Path to JSON with {'messages': [{'neutral','ru',...}, ...]}",
                    default="")
    ...
    print("Running EN→ru bias pipeline with the following settings:")
